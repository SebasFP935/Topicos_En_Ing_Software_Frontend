    ...
    <nav style={{ position: 'fixed', bottom: 10, left: 10, right: 10, background: 'rgba(10,13,18,.9)', border: `1px solid ${C.border}`, backdropFilter: 'blur(10px)', borderRadius: 18, display: 'flex', paddingBottom: 8, paddingTop: 8, zIndex: 100, boxShadow: '0 10px 24px rgba(0,0,0,.34)' }}>
      {items.map(({ to, Icon, label }) => (
        ...
        >
          {({ isActive }) => (
            <>
              <div style={{ width: 40, height: 28, borderRadius: 10, border: isActive ? `1px solid ${C.accent}66` : '1px solid transparent', background: isActive ? 'linear-gradient(130deg, rgba(255,77,109,.22), rgba(123,165,255,.16))' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }}>
