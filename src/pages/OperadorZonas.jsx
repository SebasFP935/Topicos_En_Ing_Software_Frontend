...
const TIPO_COLOR = { CUBIERTO: '#0068b7', DESCUBIERTO: '#7ba5ff', TECHADO: '#ffcc00' }
...
const SOFT_ACTIVE_BG = 'linear-gradient(138deg, rgba(0,104,183,.26) 0%, rgba(123,165,255,.22) 100%), #121722'
...
function ModalNuevaSede({ onClose, onCreada }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(auth.message(d?.mensaje, 'Error al crear la sede'))
      onCreada(d)
    } catch (e) { setError(auth.message(e?.message, 'Error al crear la sede')) }
    finally { setSaving(false) }
  }

  return (
    <Modal title="Nueva sede" onClose={onClose}>
      {error && <div style={{ background: '#0068b714', border: '1px solid #0068b740', borderRadius: 12, padding: '10px 13px', marginBottom: 15, color: '#ffcc00', fontSize: 13, fontFamily: FF }}>{error}</div>}
      <Field label="Nombre *">
...
function ModalNuevaZona({ sedeId, onClose, onCreada }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const d = await auth.readJson(res, {})
      if (!res.ok) throw new Error(auth.message(d?.mensaje, 'Error al crear la zona'))
      onCreada(d)
    } catch (e) { setError(auth.message(e?.message, 'Error al crear la zona')) }
    finally { setSaving(false) }
  }
